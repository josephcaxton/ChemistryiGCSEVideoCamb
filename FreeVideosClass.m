...
- (void)viewDidLoad {
    ...
    NSString *queryFeed = [NSString stringWithFormat:@"%@/iosStreamv2/Chemistry/ChemistryiGCSECamb.xml",domain];
    NSString *Dir = [appDelegate.applicationDocumentsDirectory stringByAppendingPathComponent:@"ChemistryiGCSECamb.xml"];
...
-(void)MyParser:(NSString *)FileLocation{
	...
	for(int idx = 0; idx < pointStrings.count - 1; idx++)
	{
        ...
        obj.ProductID = @"ChemistryiGCSECambiPad";
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if(section == 0){
        ...
        if([filteredArrayofConfigObjects count] > 0 && FullSubscription == FALSE ){
            sectionHeader.text = @"iGCSE Chemistry Cambridge – Start today from only £1.49";
        }
        else if ([filteredArrayofConfigObjects count] > 0 && FullSubscription == TRUE ){
            sectionHeader.text = @"My Courses – iGCSE Chemistry Cambridge";
        }
        else{
            sectionHeader.text =@"";
        }
